...
```text
  ...
  └──────────────┬───────────────┘
                 │
                 ▼ [Paste into "STIG Deviation Justification Engineer" Gemini Gem]
  ┌──────────────────────────────┐
...
* **Context Aggregation & Packet Formulation (`gen_ai_packets.py`):** Scans the `Exception Ledger` to isolate findings flagged with `Needs AI Draft? = TRUE`. It groups findings by **IP Address/Asset** to minimize prompt context fatigue, pulls legacy justification text blocks, and outputs clean `.md` packets. Features a graphical folder picker and an automatic `PermissionError` catch loop that lets you close open Excel instances and retry seamlessly.
* **Manual Gemini Gem Drafting Stage:** Each `.md` packet is pasted directly into the **STIG Deviation Justification Engineer** Gemini Gem (instructions mirrored locally in `prompts/gem_justificationEngineer_instructions.md` / `prompts/stig_deviation_justification_engine.md`). The Gem drafts addendum language for `Justifications for Exemptions` and `Mitigating Controls`, cites its source references and assumptions, and returns the response as structured JSON, which you save into `ai_responses/`. There is currently no automated API-calling script for this stage — it is intentionally kept manual/human-in-the-loop.
...
```bash
python scripts/gen_ai_packets.py
```

#### 3. Drafting via the Gemini Gem (Manual)
*Ensure that processing this data on cloud platforms matches your agency's information security standard tier constraints.*

This stage is deliberately manual, not an automated API call:

1. Create/open the **STIG Deviation Justification Engineer** Gem in Gemini, using the instructions in `prompts/gem_justificationEngineer_instructions.md` (or the extended version in `prompts/stig_deviation_justification_engine.md`).
2. Open a packet from `ai_packets/` and paste its contents into the Gem.
3. Save the Gem's structured JSON response into `ai_responses/<IPADDRESS>_response.json`.
...
```bash
python scripts/import_ai_responses.py
```
...
Use the unified entry point (`scripts/run_catt_unified.py`) when the scan source may be either Tenable Security Center (TSC) or a local Nessus export. `scripts/run_catt.py` is a thin alias that re-exports the same `main()`, so either invocation works:

```powershell
python -m scripts.run_catt_unified
# or, equivalently:
...
├── outputs/
├── prompts/
│   ├── gem_mergeEngineer_instructions.md          # Gemini Gem instructions for the merge/consolidation stage
│   ├── gem_justificationEngineer_instructions.md  # Gemini Gem instructions for the justification-drafting stage
│   └── stig_deviation_justification_engine.md     # Extended local mirror of the justification engine prompt
├── raw_extracted_data/
├── scripts/
│   ├── merge_stigs.py            # Consolidation engine (Local/Google Sheets ingestion)
│   ├── gen_ai_packets.py         # Context packager (Asset-grouped markdown compiler)
│   ├── import_ai_responses.py    # Data ingest utility (Safe AI review track mapper)
|   ├── api_extractor.py
|   ├── catt_engine.py
|   ├── csv_extractor.py
|   ├── isso_analyzer.py
|   ├── nessus_api_client.py
|   ├── run_catt.py               # Thin alias for run_catt_unified.main()
|   ├── run_catt_unified.py       # Unified TSC/Nessus download + extraction entry point
|   └── tsc_auth_client.py
...
│   ├── <IPADDRESS>_response.json   # Structural outputs captured by stage 3 (manual Gemini Gem drafting)
│   └── errors/                      # Quarantine silo folder for invalid payloads
├── stig_deviation_ai_justification_workflow.md  # Design/planning doc for the AI justification workflow
